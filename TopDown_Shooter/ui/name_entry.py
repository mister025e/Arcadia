...
class NameEntryScreen:
    ...
    def __init__(self, ui_parent, on_finish):
        ...
        self.name_instr = Text(
            parent=ui_parent,
            text='Use Stick Up/Down to change letter,\nLeft/Right to change slot,\nButton 0 to confirm',
            position=(0, 0.4),
            origin=(0, 0),
            scale=1,
            color=color.white,
    ...
    def input(self, key):
        """
        Called from GameManager.input() or via joystick proxy from update().
        Respond to:
          - 'w': cycle letter upward
          - 's': cycle letter downward
          - 'd': move to next slot (if that slot isn’t blank)
          - 'a': move to previous slot
          - 'space': confirm entry (finish)
        """
    ...
    def _finish_entry(self):
        ...
        then call the callback (to save and return to game over).
        """
        i = self.current_slot_index
