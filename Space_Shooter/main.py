...
for i in range(pygame.joystick.get_count()):
    ...
    sticks.append(js)
stick1 = sticks[0] if len(sticks) > 0 else None
stick2 = sticks[1] if len(sticks) > 1 else None
...
def update():
    ...
    # ── pump joystick events ────────────────────────────────
    for e in pygame.event.get():
        # map button 6→'q', 7→'z', others→any‐key
    ...
    if GameState.current == 'play':
        # ----- Mouvement joueur -----
        players_input(player, player2, cam1, cam2, focus_circle_1, focus_circle_2, joy1=stick1, joy2=stick2)
